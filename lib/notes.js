...
// One-time bootstrap. Runs at every server start; protected by a
// hidden marker file so the legacy migration only fires once. The
// welcome refresh is unconditional — it always overwrites
// notesDir/<welcomeId>.json from the repo template, so each server
// release can ship a "what's new" section in the welcome and the
// next restart picks it up. User edits to the welcome revert; user
// deletes return; that's intentional — welcome is the server's
// release-notes channel, not user content.
//
//   legacyNotesDir   path to the old <repo>/notes directory; if it
//                    exists and has *.json files and the marker is
//                    missing, those files are copied to notesDir.
//   templatePath     absolute path to the welcome template file in
//                    the repo (templates/welcome-note.json).
function ensureSeeded(notesDir, legacyNotesDir, templatePath) {
  ensureDir(notesDir)
  const markerPath = path.join(notesDir, '.seeded')

  if (!fs.existsSync(markerPath)) {
    // First-ever launch on this host (or on this notesDir). Migrate
    // any legacy in-repo notes over before we touch the welcome —
    // a user-edited welcome from the old location should win over
    // the template until the next restart.
    if (legacyNotesDir && fs.existsSync(legacyNotesDir)) {
      let copied = 0
      for (const file of fs.readdirSync(legacyNotesDir)) {
        if (!file.endsWith('.json')) continue
        const id = file.slice(0, -'.json'.length)
        if (!isValidId(id)) continue
        const src = path.join(legacyNotesDir, file)
        ...
        try {
          ...
          console.warn(`[notes] migrate ${file} failed: ${err.message}`)
        }
      }
      if (copied > 0) {
        console.log(`[notes] migrated ${copied} legacy note${copied !== 1 ? 's' : ''} from ${legacyNotesDir} to ${notesDir}`)
      }
    }
    try {
      fs.writeFileSync(markerPath, new Date().toISOString())
    } catch (err) {
      console.warn(`[notes] failed to write seed marker: ${err.message}`)
    }
  }

  // Always-overwrite welcome from template. If the template can't
  // be read, leave any existing welcome alone — better to ship a
  // possibly-stale welcome than to wipe one with a server-side
  // template error.
  if (templatePath && fs.existsSync(templatePath)) {
    try {
      const raw = fs.readFileSync(templatePath, 'utf8')
      const parsed = JSON.parse(raw)
      if (parsed && isValidId(parsed.id)) {
        const dst = path.join(notesDir, `${parsed.id}.json`)
        fs.writeFileSync(dst, JSON.stringify(parsed, null, 2), 'utf8')
      }
    } catch (err) {
      console.warn(`[notes] welcome refresh from ${templatePath} failed: ${err.message}`)
    }
  }
}
