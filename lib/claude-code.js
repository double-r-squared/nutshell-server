...
// Resolve the installed @anthropic-ai/claude-agent-sdk version by
// reading its package.json. Returns null if the SDK isn't installed
// or the package.json can't be read. Used by /claude-code/status so
// the phone can display the version inline (matches how the server
// version is shown next to the server name).
function resolveSdkVersion() {
  try {
    const pkgPath = require.resolve('@anthropic-ai/claude-agent-sdk/package.json')
    // require() caches, but we want the freshest value after an
    // sdk-update endpoint hit — use fs.readFileSync to bypass cache.
...
async function streamClaudeCode({
  ...
  // when empty.
  appendSystemPrompt,
  // SDK model id ('claude-opus-4-7' / 'claude-sonnet-4-6' /
  // 'claude-haiku-4-5-20251001'). Pass-through to options.model;
  // when null/undefined the SDK picks its own default.
  model,
}) {
  ...
  // Tracks whether stream_event text_deltas were observed for the
  // current assistant message. When they arrive we use them
  // (live token streaming) and skip the assistant message's text
  // block at the end. When they DON'T arrive — happens on some
  // resumed sessions; SDK doesn't always emit partials — we fall
  // back to emitting the full text block so the phone shows the
  // reply instead of an empty turn. Resets every time a new
  // assistant message lands.
  let messageHadPartials = false
  ...
  for await (const msg of q) {
    ...
    if (msg.type === 'assistant') {
      ...
      for (const block of blocks) {
        if (block.type === 'text' && typeof block.text === 'string') {
          if (!messageHadPartials) {
            // No stream_event partials observed for this assistant
            // message — emit the full text block so we don't ship
            // an empty reply. Common on resumed CC sessions where
            // the SDK skips the partial-messages stream.
      ...
      // Reset the partial-detector for the next assistant message in
      // this turn (rare but happens when tool calls split the stream
      // into pre- and post-tool messages).
      messageHadPartials = false
      continue
    }
    if (msg.type === 'user') {
      const blocks = (msg.message && msg.message.content) || []
...
// ── Usage aggregation ───────────────────────────────────────────────────────
//
// Scan every JSONL under ~/.claude/projects, extract usage blocks from
// assistant messages, multiply by model pricing, and return totals
// bucketed into the two windows the phone's Usage page renders:
//   - 5-hour rolling window from now (Anthropic's rate-limit "block")
//   - 7-day rolling window from now (weekly cap)
//
// Pricing per million tokens (USD), 2026 rates. cache_creation = 1.25x
// input, cache_read = 0.1x input. Unknown models fall back to Sonnet
// pricing — conservative middle ground that won't undercount.
const USAGE_PRICES = {
...
function priceForModel(model) {
  ...
  if (m.includes('haiku')) return USAGE_PRICES.haiku
  if (m.includes('opus')) return USAGE_PRICES.opus
  return USAGE_PRICES.sonnet
}
...
// Sum cost across every assistant turn whose timestamp falls inside
// each window. Returns { session: {...}, week: {...} } where each
// bucket carries spent_usd, tokens_in, tokens_out, and the boundary
// the phone uses to compute pct against its configured plan limit.
function summarizeUsage() {
  const now = Date.now()
  const sessionStart = now - 5 * 60 * 60 * 1000
  const weekStart = now - 7 * 24 * 60 * 60 * 1000
  // `messages` is the headline metric on the phone — Anthropic publishes
  // their subscription rate-limits in messages-per-window, not dollars,
  // so message count is the most honest progress indicator. We keep
  // `spent_usd` alongside for raw-cost surfacing (debug, comparison
  // with OpenRouter) and for API-mode users who care about dollars.
  const buckets = {
    session: { spent_usd: 0, tokens_in: 0, tokens_out: 0, messages: 0 },
    week: { spent_usd: 0, tokens_in: 0, tokens_out: 0, messages: 0 },
  }
  if (!fs.existsSync(PROJECTS_DIR)) {
    return { ...buckets, generatedAt: now, sessionStart, weekStart }
  }
  let projectDirs
  try {
    projectDirs = fs.readdirSync(PROJECTS_DIR)
  ...
  for (const projectDir of projectDirs) {
    ...
    for (const file of files) {
      if (!file.endsWith('.jsonl')) continue
      const filePath = path.join(dir, file)
      // Skip files whose mtime is older than the week window —
      // they can't contribute to either bucket and parsing them
      // would waste IO.
      try {
        const fstat = fs.statSync(filePath)
        if (fstat.mtimeMs < weekStart) continue
      } catch { continue }
      let raw
      ...
      for (const line of raw.split('\n')) {
        ...
        const cost = costFromUsage(usage, record.message.model)
        if (ts >= weekStart) {
          buckets.week.spent_usd += cost
          buckets.week.tokens_in += usage.input_tokens || 0
          buckets.week.tokens_out += usage.output_tokens || 0
          buckets.week.messages += 1
        }
        if (ts >= sessionStart) {
          buckets.session.spent_usd += cost
          buckets.session.tokens_in += usage.input_tokens || 0
          buckets.session.tokens_out += usage.output_tokens || 0
          buckets.session.messages += 1
        }
      }
    }
  }
