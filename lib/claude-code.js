...
async function streamClaudeCode({
  ...
  // TEMP debug counters — surface what the SDK actually produces for
  // this turn. Logged once at end so we can see whether 'assistant'
  // text blocks came through or whether the turn was pure tool-use /
  // empty. Remove after multi-session-empty-reply bug is closed.
  const dbg = { stream_event_text: 0, assistant_text_blocks: 0, assistant_tool_use: 0, user_tool_result: 0, result: 0, other: 0, fallback_emits: 0 }
  ...
  console.log(
    `[cc-debug] msg counts: stream_text=${dbg.stream_event_text} assistant_text=${dbg.assistant_text_blocks} (fallback_emits=${dbg.fallback_emits}) tool_use=${dbg.assistant_tool_use} tool_result=${dbg.user_tool_result} result=${dbg.result} other=${dbg.other}`,
  )
}
